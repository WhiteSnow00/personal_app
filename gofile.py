...
class Downloader:
    def __init__(
        ...
        self._content_dir: str = ""
        self._success_file_path: str = ""
    ...
    def run(self) -> None:
        ...
        self._success_file_path = path.join(self._content_dir, "success.txt")
        
        if path.exists(self._success_file_path):
            try:
                with open(self._success_file_path, "r") as f:
                    self._downloaded_files = set(line.strip() for line in f if line.strip())
            except Exception as e:
                _print(f"Warning: Could not read success.txt: {e}{NEW_LINE}")
    ...
    def _download_content(self, file_info: dict[str, str]) -> None:
            ...
            try:
                relative_path = path.relpath(filepath, self._content_dir)
            except ValueError:
                relative_path = filepath

            if relative_path in self._downloaded_files:
                _print(f"{filepath} found in success.txt (already downloaded), skipping.{NEW_LINE}")
                return
    ...
    def _should_skip_download(self, filepath: str) -> bool:
        if path.exists(filepath) and path.getsize(filepath) > 0:
            _print(f"{filepath} already exist on disk, skipping.{NEW_LINE}")
            return True
        return False
    ...
    def _finalize_download(self, file_info: dict[str, str], tmp_file: str, has_size: str) -> None:
        if path.getsize(tmp_file) == int(has_size):
            ...
            final_path = path.join(file_info["path"], file_info["filename"])
            move(tmp_file, final_path)
            
            try:
                relative_path = path.relpath(final_path, self._content_dir)
                with open(self._success_file_path, "a") as f:
                    f.write(f"{relative_path}{NEW_LINE}")
                self._downloaded_files.add(relative_path)
            except Exception:
                pass
